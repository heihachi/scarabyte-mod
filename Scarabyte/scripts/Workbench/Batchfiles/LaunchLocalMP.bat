...
:: Mods
::echo start %serverEXE% %serverLaunchParams% -scrAllowFileWrite -config=serverDZ.cfg -port=%port% "-profiles=%serverProfileDirectory%" -dologs -adminlog -freezecheck -scriptDebug=true -cpuCount=4 "-mission=%mission%" "-mod=%modList%"
::start %serverEXE% %serverLaunchParams% -scrAllowFileWrite -config=serverDZ.cfg -port=%port% "-profiles=%serverProfileDirectory%" -dologs -adminlog -freezecheck -scriptDebug=true -cpuCount=4 "-mission=%mission%" "-mod=%modList%"
:: No mods
echo start %serverEXE% %serverLaunchParams% -scrAllowFileWrite -config=serverDZ.cfg -port=%port% "-profiles=%serverProfileDirectory%" -dologs -adminlog -freezecheck -scriptDebug=true -cpuCount=4 "-mission=%mission%"
start %serverEXE% %serverLaunchParams% -scrAllowFileWrite -config=serverDZ.cfg -port=%port% "-profiles=%serverProfileDirectory%" -dologs -adminlog -freezecheck -scriptDebug=true -cpuCount=4 "-mission=%mission%"

TIMEOUT /T 5 /NOBREAK

chdir /d "%gameDirectory%"
:: Mods
::echo start %clientEXE% %clientLaunchParams% -connect=127.0.0.1 -port=%port% "-mod=%modList%" -dologs -adminlog -freezecheck -scriptDebug=true
::start %clientEXE% %clientLaunchParams% -connect=127.0.0.1 -port=%port% "-mod=%modList%" -dologs -adminlog -freezecheck -scriptDebug=true
:: No mods
echo start %clientEXE% %clientLaunchParams% -connect=127.0.0.1 -port=%port% -dologs -adminlog -freezecheck -scriptDebug=true
start %clientEXE% %clientLaunchParams% -connect=127.0.0.1 -port=%port% -dologs -adminlog -freezecheck -scriptDebug=true
pause